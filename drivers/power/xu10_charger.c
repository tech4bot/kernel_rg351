					   ...
					   int input_current)
{
	...
	DBG("rk817_charge_set_input_current cuurent=%d \n",input_current);
}
					...
					enum charger_t charger)
{
	...
	case USB_TYPE_USB_CHARGER:
		charge->usb_in = 1;
		charge->ac_in = 0;
		if (rk817_charge_get_dsoc(charge) >= 85)
    ...
    if(charge->prop_status==POWER_SUPPLY_STATUS_CHARGING){
	   if(charge->pdata->chg_full_led_pin)
	     gpio_set_value(charge->pdata->chg_full_led_pin,0);
	   if(charge->pdata->chg_led_pin)
	     gpio_set_value(charge->pdata->chg_led_pin,1);
    }
    else if(charge->prop_status==POWER_SUPPLY_STATUS_FULL){
	   if(charge->pdata->chg_led_pin)
	     gpio_set_value(charge->pdata->chg_led_pin,0);
	   if(charge->pdata->chg_full_led_pin)
	     gpio_set_value(charge->pdata->chg_full_led_pin,1);
    }
	else{
	   if(charge->pdata->chg_led_pin)
	     gpio_set_value(charge->pdata->chg_led_pin,0);
...
static void rk817_charge_led_worker(struct work_struct *work)
{
	...
	if (!rk817_charge_online(charge)){
		if(rk817_charge_get_dsoc(charge) <= 20){
		 charge->bat_low=true;
		 if(charge->pdata->chg_led_pin)
	       gpio_set_value(charge->pdata->chg_led_pin,!gpio_get_value(charge->pdata->chg_led_pin));
		}
		else if(charge->bat_low){
		 charge->bat_low=false;
		 if(charge->pdata->chg_led_pin)
		   gpio_set_value(charge->pdata->chg_led_pin,0);	
		}
	}
	else{
		if(rk817_charge_get_dsoc(charge)>=85 && charge->prop_status!=POWER_SUPPLY_STATUS_FULL){
		  if(charge->usb_in)
		  	rk817_charge_set_chrg_param(charge,USB_TYPE_USB_CHARGER);
		  else if(charge->ac_in)
		  	rk817_charge_set_chrg_param(charge,USB_TYPE_AC_CHARGER);
		  else if(charge->dc_in)
		  	rk817_charge_set_chrg_param(charge,DC_TYPE_DC_CHARGER);
		}
	}
	queue_delayed_work(charge->led_wq, &charge->led_work,msecs_to_jiffies(1000));
}
#endif
...
static int rk817_charge_parse_dt(struct rk817_charger *charge)
{
...
#if defined(CONFIG_ARCH_ROCKCHIP_ODROIDGOA)
	if (!of_find_property(np, "chg-led-gpio", &ret)) {
	...
	if (!of_find_property(np, "chg-full-led-gpio", &ret)) {
		DBG("not support charge full led\n");
		pdata->chg_full_led_pin = 0;
	} else {
		pdata->chg_full_led_pin = of_get_named_gpio_flags(np,
						"chg-full-led-gpio",
						0, &flags);
		if (gpio_is_valid(pdata->chg_full_led_pin)) {
			DBG("support charge full led\n");
		}
	}
	
	if (!of_find_property(np, "otg-pwr-gpio", &ret)) {
		pr_err("%s() not found otg pwr pin! \n",__func__);
		pdata->otg_pwr_pin = 0;
	} else {
		pdata->otg_pwr_pin = of_get_named_gpio_flags(np,
						"otg-pwr-gpio",
						0, &flags);
		if (gpio_is_valid(pdata->otg_pwr_pin)) {
			ret = devm_gpio_request(dev, pdata->otg_pwr_pin, "otg-pwr");
			if (ret < 0) {
				pr_err("%s()  request ERROR\n",
							__func__);
				return ret;
			}
			/* set gpio to low level */
			ret = gpio_direction_output(pdata->otg_pwr_pin , 0);
			if (ret < 0) {
					pr_err("%s() set  otg pwr pin error !\n",__func__);
					return ret;
			}	
			DBG("support otg pwr pin!\n");
		}
		else{
		   pr_err("%s() not support otg pwr pin! \n",__func__);
		}
	}
#endif
...
static void rk817_charge_irq_delay_work(struct work_struct *work)
{
	...
	if (charge->plugin_trigger) {
		...
		if(charge->otg_in){
		 rk817_charge_set_chrg_param(charge, USB_TYPE_AC_CHARGER);
		 if(charge->pdata->otg_pwr_pin)
		   gpio_set_value(charge->pdata->otg_pwr_pin,0);
		 DBG("otg vbus pwr disable!\n");
		 return;
		}
		#endif
		if (charge->pdata->extcon)
		...
		#if defined(CONFIG_ARCH_ROCKCHIP_ODROIDGOA)
		if(charge->pdata->otg_pwr_pin)
		   gpio_set_value(charge->pdata->otg_pwr_pin,1);
		DBG("otg vbus pwr enable!\n");
		#endif
	} else {
...
static int rk817_charge_init_irqs(struct rk817_charger *charge)
{
    ...
    #if defined(CONFIG_ARCH_ROCKCHIP_ODROIDGOA)
	if(extcon_get_cable_state_(charge->cable_edev, EXTCON_USB_VBUS_EN) > 0){
	   rk817_charge_set_otg_in(charge,ONLINE);
	}
	else{
	   rk817_charge_set_otg_in(charge,OFFLINE);
...
MODULE_DESCRIPTION("xu10 charge driver");
